...
[Yy]* ) 
    ...
    sudo apt-get update
    # Install.
    sudo apt-get install -y --no-install-recommends filezilla gazebo2 geany geany-plugin-addons nautilus-open-terminal python-wstool ros-indigo-desktop synaptic terminator vlc wxmaxima
    echo 'Done.'
    main
    ;;
# Negative action
...
1)
    ...
    [Yy]* ) 
        echo 'Requires root privileges:'
        # Feel free to change to whatever suits your preferences.
        sudo apt-get install -y bzr devscripts gcc-snapshot git glade gnome-common gtk-3-examples lib32stdc++6 nodejs python-launchpadlib python3-distutils-extra scons texlive-latex-extra
        echo 'Done.'
        development
        ;;
    # Negative action
...
echo '3. Reset default font preferences'
echo 'r. Return'
echo ''
read -p 'What would you like to do? (Enter your choice) : ' REPLY
case $REPLY in
...
1)
    ...
    gsettings set org.gnome.nautilus.desktop font 'Ubuntu 12'
...
3)
    ...
    # Unity Settings
    echo 'Setting Unity preferences...'
    gsettings reset com.canonical.Unity.ApplicationsLens display-available-apps
    gsettings reset com.canonical.unity-greeter draw-user-backgrounds
    gsettings reset com.canonical.indicator.power icon-policy
    gsettings reset com.canonical.Unity.Lenses remote-content-search
    # Nautilus Preferences
    echo 'Setting Nautilus preferences...'
    gsettings reset org.gnome.nautilus.preferences sort-directories-first
    # Rhythmbox Preferences
    echo 'Setting Rhythmbox preferences...'
    gsettings reset org.gnome.rhythmbox.rhythmdb monitor-library
    gsettings reset org.gnome.rhythmbox.sources browser-views
    # Done
    echo "Done."
    config
    ;;
